...
Start with one small rendering merge, then address model defaults, navigation, missing tool outputs, and ephemeral-chat loss. Group work by shared behavior and code ownership.
...
| Issues | Scope | Size |
| --- | --- | --- |
| [#10](https://github.com/NickAwrist/euler/issues/10), [#11](https://github.com/NickAwrist/euler/issues/11), [#22](https://github.com/NickAwrist/euler/issues/22) | Restore list markers, preserve single line breaks, and open external reply links in a new tab. All live in `ui/components/MarkdownMessage.tsx`. | Small |

This has frequent user impact, a narrow implementation, and one focused rendering regression pass. Keep math rendering separate because currency handling introduces additional decisions.
...
| P3 | [#12](https://github.com/NickAwrist/euler/issues/12) | Standalone math rendering after the Markdown fixes. Include an explicit policy for dollar delimiters versus currency. |
...
The initial parallel batch can be Markdown fixes, model defaults, and Usage fixes. These have relatively little overlap.
